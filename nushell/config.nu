...
def --env setup-path [] {
    ...
    let go_bin = "~/go/bin"
    # Add ASDF shims
    let asdf_shims = "~/.asdf/shims"
    let deno_bin = "~/.deno/bin"
    let cargo_bin = "~/.cargo/bin/"
    let gradle = "$home/.sdkman/candidates/gradle/current/bin"
    # Update PATH
...
# Optional: Add Paperspace configuration similar to your zsh setup
def --env setup-paperspace [] {
    let paperspace_install = "~/.paperspace"
    let paperspace_bin = $"($paperspace_install)/bin"
    
    $env.PAPERSPACE_INSTALL = $paperspace_install
    $env.PATH = ($env.PATH | prepend $paperspace_bin)
}
...
# General
# -----------------------------------------------------
alias nf = fastfetch --config examples/13
alias ff = fastfetch
...
alias wifi = nmtui
alias codelang = cd /run/media/suresh/D/codelang/
alias conf = cd ~/.config/
alias cls = clear
alias .. = cd ..
alias .... = cd ../..
alias home = cd ~
alias exa = exa -a --icons=always
...
# Add SDKMAN! Gradle path to PATH
# -----------------------------------------------------
